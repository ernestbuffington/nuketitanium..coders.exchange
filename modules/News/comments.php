...
/* Applied rules: Ernest Allen Buffington (TheGhost) 04/29/2023 5:35 PM
...
function DisplayTopic ($sid, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
    ...
    echo "</font></td></tr><tr><td>";
    if((isset($userinfo['commentmax'])) && (strlen((string) $r_comment) > $userinfo['commentmax'])) echo substr("$r_comment", 0, $userinfo['commentmax'])."<br /><br /><strong><a href=\"modules.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a></strong>";
...
function reply($pid, $sid, $mode, $order, $thold) {
    ...
    if ($pid == 0) {
        ...
        $subject = stripslashes((string) check_html($row4["subject"], "nohtml"));
    }
    CloseTable();
    echo "<br />";
