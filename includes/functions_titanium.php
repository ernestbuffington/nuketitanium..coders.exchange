...
/* Idea came from Nuke Evo
 * @Original Author for GetColorGroups function was by JeFFb68CAM
 * @Contributor (ReOrGaNiSaTiOn) called by several files - so it made sense to cache it 
 * @author Ernest Allen Buffington 04/19/2023
 */
function GetColorGroups($in_admin = false) 
{
    ...
    if((($ColorGroupsCache = $cache->load('NukeColorGroups', 'config')) === false) || empty($ColorGroupsCache)) 
	{
        $ColorGroupsCache = '';
		...
		{
		  $get_the_right_group_id = $db->sql_query("SELECT `nuke_group_id` FROM `".GROUPS_TABLE."` WHERE `group_name`='$group_name'");
		  list($proper_nuke_group_id) = $db->sql_fetchrow($get_the_right_group_id);
		  $ColorGroupsCache .= '&nbsp;[&nbsp;<strong><a href="modules.php?name=Groups&g='.$proper_nuke_group_id.$back.'"><span class="genmed" style="color:#'. $group_color .';">'. $group_name .'</span></a></strong>&nbsp;]&nbsp;';
        }
        $db->sql_freeresult($result);
        $cache->save('NukeColorGroups', 'config', $ColorGroupsCache);
    }
